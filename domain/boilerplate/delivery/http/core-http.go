...
func (handler *HttpBoilerplateHandler) GetAll(ctx *gin.Context) {
	...
	if err != nil {
		if err.Error() == config.SQL_NOT_FOUND {
			message.ReturnOk(ctx, make(map[string]interface{}), status, err.Error())
			return
		}
	...
	message.ReturnOk(ctx, response, status, msg)
}
...
func (handler *HttpBoilerplateHandler) Store(ctx *gin.Context) {
	...
	message.ReturnSuccessInsert(ctx, feedback.Data, status, msg)
}
